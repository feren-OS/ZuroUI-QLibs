...
#include "zurouiswitch.h"
#include <tvariantanimation.h>
#include <QPaintEvent>
#include <QPainter>
#include <QStaticText>

zurouiSwitch::zurouiSwitch(QWidget* parent) : QPushButton(parent) {
...
QPalette::ColorGroup zurouiSwitch::IsActiveColorRole() {
    if (this->isEnabled()) {
...
void zurouiSwitch::paintEvent(QPaintEvent* event) {
...
void zurouiSwitch::mousePressEvent(QMouseEvent* event) {
    mouseClickPoint = event->localPos().toPoint().x();
    initialPoint = mouseClickPoint;
}

void zurouiSwitch::mouseMoveEvent(QMouseEvent* event) {
...
void zurouiSwitch::mouseReleaseEvent(QMouseEvent* event) {
...
void zurouiSwitch::checkChanging(bool checked) {
...
QSize zurouiSwitch::sizeHint() const {
    QFontMetrics metrics(this->font());
    int width = SC_DPI(33);
    if (iText == "") {
        width += SC_DPI(8);
...
QString zurouiSwitch::OnText() {
    return iText;
}

QString zurouiSwitch::OffText() {
    return oText;
}

void zurouiSwitch::setOnText(QString text) {
    iText = text;
    this->iIcon = QIcon();
    this->setFixedSize(this->sizeHint());
    this->update();
...
void zurouiSwitch::setOffText(QString text) {
    oText = text;
    this->setFixedSize(this->sizeHint());
    this->update();
}

void zurouiSwitch::setOnIcon(QIcon icon) {
    this->iIcon = icon;
    iText = "";
    this->setFixedSize(this->sizeHint());
    this->update();
...
QIcon zurouiSwitch::OnIcon() {
    return this->iIcon;
}

void zurouiSwitch::setChecked(bool checked) {
