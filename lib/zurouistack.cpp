...
#include "zurouistack.h"

class zurouiStackPrivate {
    public:
        zurouiStackPrivate(zurouiStack* parent) {
            this->parent = parent;
        }
        ...
        zurouiStack::Animation anim = zurouiStack::None;
        QAbstractAnimation* currentAnimation = nullptr;

        zurouiStack* parent;
};

zurouiStack::zurouiStack(QWidget *parent) : QStackedWidget(parent)
{
    d = new zurouiStackPrivate(this);
}

zurouiStack::~zurouiStack() {
    delete d;
}

void zurouiStack::setCurrentIndex(int index, bool doAnimation) {
    if (doAnimation) {
...
void zurouiStack::doSetCurrentIndex(int index) {
        ...
        if (nextWidget == nullptr) {
            ...
            switch (d->anim) {
                ...
                case SlideHorizontal: {
                    ...
                    connect(this, &zurouiStack::resized, animation, [=] {
                        animation->setEndValue(QRect(0, 0, this->width(), this->height()));
                    });
...
void zurouiStack::setCurrentWidget(QWidget *w, bool doAnimation) {
    this->setCurrentIndex(indexOf(w), doAnimation);
}

zurouiStack::Animation zurouiStack::CurrentAnimation() {
    return d->anim;
}

void zurouiStack::setCurrentAnimation(Animation animation) {
    d->anim = animation;
    emit CurrentAnimationChanged(animation);
}

int zurouiStack::addWidget(QWidget* w) {
    w->setAutoFillBackground(true);
    return QStackedWidget::addWidget(w);
}

int zurouiStack::insertWidget(int index, QWidget* w) {
    w->setAutoFillBackground(true);
    return QStackedWidget::insertWidget(index, w);
}

void zurouiStack::resizeEvent(QResizeEvent *event) {
    emit resized();
}
